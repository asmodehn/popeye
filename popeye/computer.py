...
class Process:
    ...
    class Type:
        ...
        """
        repr: str  # representation potentially different from program implementation
        elem: Process.Expression

        def __init__(self, elem: Process.Expression, repr = None):
            self.elem = elem
            self.repr = repr if repr is not None else elem.repr
        ...
        def __getitem__(self, item: Process.Type):
            """
            Type-index of Type Family
            :param item:
            :return:
            """
            # we need to call elem with the item to retrieve the type family
            return Process.Type( self.elem(item.elem) )
    ...
    class Bool(Type):
        ...
        def __eq__(self, other):
            """
            Need to evaluate more (down to python implementation level) to check equality using python equality.
            :param other:
            :return:

            Note : we could implement equality in many other ways... but it has to match our evaluation strategy somehow...
            """
            try:
